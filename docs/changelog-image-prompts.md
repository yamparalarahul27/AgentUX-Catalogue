...
Release titles below are kept in sync with `designer/public/whats-new.json`.
...
- **Empty "comment removed" placeholders self-clean** — `a pale frosted-glass placeholder bar folding softly into the dark background, leaving the thread clean`
- **Comments — edit your own, reply to a thread** — `a frosted-glass comment card with a tiny edit mark and one indented reply card tucked beneath it`
- **Header tabs reordered** — `a calm frosted-glass tab strip with three simple tab markers shifted into a new order`
...
- **Edit offline — your changes sync when you reconnect** — `a frosted-glass status pill with a circular arrow, a small stack of cards queued behind it`
...
- **Lightbox: resize handles + UI-E tab + cleaner Comments view** — `a frosted-glass image with a rectangular selection frame, small grips at its corners and a quiet comments panel beside it`
...
- **AI labelling now suggests element locations** — `a frosted-glass screenshot with a dashed rectangular outline marking a suggested region`
- **Labelling Studio — three fixes** — `a clean frosted-glass grid of cards in tidy paginated rows`
- **Labelling Studio — lightbox opens correctly on every card** — `a single frosted-glass card opening into a larger centered image`
- **Catalogue cards now join the splash fall-in + crisper empty-state art** — `frosted-glass cards gently settling into a grid from above with a soft tilt`
- **Catalogue lands with a flourish — plus polish** — `a frosted-glass header bar, chip strip and first cards descending into place from above`
- **Refresh toast — rainbow ring + shorter snooze** — `a frosted-glass toast card with one slim, quiet ring around its edge`
...
- **Lightbox edit — group dropdown now shows brand icons** — `a frosted-glass dropdown list where each row carries a small brand-dot beside the name`
...
- **Videos — X (Twitter) and Family Values split into tabs** — `a frosted-glass panel split by two tab markers at the top, one active`
- **Videos — preview locks background scroll on mobile** — `a frosted-glass phone with a centered preview panel, the background held still behind it`
...
- **Lightbox polish — Figma-quality squircle corners** — `a frosted-glass button row with smooth, continuous squircle corners`
- **Coverage score — see at a glance which apps are well-captured** — `a frosted-glass card with two slim horizontal progress bars at different fill levels`
...
- **Search — clearer, clearable, and Enter goes where you'd expect** — `a frosted-glass search bar with a clean result list and one highlighted match`
...
- **New loading indicator across buttons + spinners** — `a set of concentric diamond ripples in frosted glass, a quiet loading pulse`
- **Mobile viewport fits properly now** — `a frosted-glass phone where the panel fits perfectly inside the visible screen`
...
- **Group detail pages, login backdrop & toolbar refinements** — `a frosted-glass page tile showing a tight grid of small screens`
- **Role system, Marketing Bucket & capability-aware UI** — `a frosted-glass key shape paired with a small set of capability dots`
...
- **Telegram Bot, X Video Links & Feature Log Improvements** — `a frosted-glass paper-plane mark sending a small image card toward a grid`
- **Feature Log Lifecycle, Global Scope & Navigation Restructure** — `a frosted-glass board of cards arranged in three tidy columns`
- **Folder View, Group Appearance & Gallery Workflow** — `a frosted-glass folder card with a small carousel of preview tiles`
- **Compare Mode, Team Flows & CD Page Experiments** — `two frosted-glass image panels side by side for comparison`
