...
| **Video lightbox doesn't lock body scroll** — opening the video preview lets the catalogue page scroll behind it. `Catalogue.tsx`'s `isAnyModalOpen` covers `previewFamily` + uploads + settings, but the video preview state (`previewItemKey` at `CatalogueVideosSection.tsx:996`) is local to the section, so it's missed | Reported 2026-05-27 during mobile QA; lift state into Catalogue OR apply scroll-lock inside the section component | new — file `parked_video_lightbox_body_scroll.md` when picked up |
