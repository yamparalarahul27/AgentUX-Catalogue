...
// Pull a human title from the prototype's own <title>. Falls back to a
// generic label when the upload has no (or an empty) title.
function extractTitle(html: string): string {
  const match = html.match(/<title[^>]*>([\s\S]*?)<\/title>/i);
  const cleaned = (match ? match[1] : '').replace(/\s+/g, ' ').trim().slice(0, 120);
  return cleaned || 'Interactive Prototype';
}

// Inject Open Graph + Twitter meta so shared prototype links unfurl with
// a branded card + the prototype's real title (instead of a blank preview).
// Only ADDS tags — never strips a prototype's own head content.
export function injectOgMeta(html: string): string {
  const title = extractTitle(html);
  const meta = [
    `<meta property="og:type" content="website" />`,
    `<meta property="og:site_name" content="AgentUX" />`,
  ...
  // Malformed HTML with no <head>: synthesise one after <html …>, else prepend.
  const htmlOpen = html.match(/<html\b[^>]*>/i);
  if (htmlOpen) {
    return html.replace(htmlOpen[0], `${htmlOpen[0]}<head>\n${meta}\n</head>`);
  }
  return `<head>\n${meta}\n</head>${html}`;
}
